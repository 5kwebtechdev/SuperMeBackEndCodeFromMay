...
public class AdminS3Controller {
    ...
    public PresignResponseDto presign(@RequestHeader("Authorization") String token,@RequestBody PresignRequestDto dto) {
        Long adminId = AdminJwtUtil.getAdminIdFromToken(token.replace("Bearer ", ""));
        dto.setAdminId(adminId);
        dto.setRole(AdminJwtUtil.extractRole(token.replace("Bearer ", "")));
    ...
    public List<PresignResponseDto> presignMultiple(
            ...
            Long adminId = AdminJwtUtil.getAdminIdFromToken(token.replace("Bearer ", ""));
            String role = AdminJwtUtil.extractRole(token.replace("Bearer ", ""));
        return s3Service.createMultiplePresigned(adminId,role, dtos);
    }
    ...
    public List<FileUpload> filesByUser(@RequestHeader("Authorization") String token) {
        Long adminId = AdminJwtUtil.getAdminIdFromToken(token.replace("Bearer ", ""));
        String role = AdminJwtUtil.extractRole(token.replace("Bearer ", ""));
        return s3Service.getFilesByUser(adminId,role);
    }
